...
{
    // search if the number is in the list
    node<ITEM_TYPE> *walker = _search_list(head, item);
    if (walker != nullptr)
        walker->_item *= 2; // found

    else
        _insert_sorted(head, item);
}
