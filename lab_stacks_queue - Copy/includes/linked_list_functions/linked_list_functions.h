...
// these will be now turned into doubly linked lists
...
// CHANGED FOR DOUBLY LINKED LIST
template <typename ITEM_TYPE>
node<ITEM_TYPE> *_insert_head(node<ITEM_TYPE> *&head, ITEM_TYPE insert_this)
{

    // 1. create a new node  // 2. _next point the same as headptr & _prev node is always nullptr
    node<ITEM_TYPE> *temp = new node<ITEM_TYPE>(insert_this, head, nullptr);

    // if the list is not empty, there is an extra step
    if (head)
    {
        head->_prev = temp;
    }
...
node<ITEM_TYPE> *_insert_after(node<ITEM_TYPE> *&head, node<ITEM_TYPE> *after_this, ITEM_TYPE insert_this)
{
    ...
    node<ITEM_TYPE> *temp = new node<ITEM_TYPE>(insert_this, after_this->_next, after_this);

    // readjust pointers of surroundings
    if (temp->_next)
    {
...
node<ITEM_TYPE> *_previous_node(node<ITEM_TYPE> *head, node<ITEM_TYPE> *prev_to_this)
{
    ...
    node<ITEM_TYPE> *walker = prev_to_this->_prev;
    return walker;
}
...
{
    // empty list -> just add it
    // start of the list -> just insert head
    ...
    // create the new node
    node<ITEM_TYPE> *temp = new node<ITEM_TYPE>(insert_this, before_this, before_this->_prev);
...
ITEM_TYPE _delete_node(node<ITEM_TYPE> *&head, node<ITEM_TYPE> *delete_this)
{
    ...
    if (head == delete_this)
    {
        ...
        // head is now on the second node
        if (head)
        {
            head->_prev = nullptr;
        }
